...
def send_plain_mail(subject: str, message: str, from_: str, to: List[str]):
    ...
    try:
        # Filter valid emails
        valid_emails = [
            email for email in to if email and not email.startswith("noemail")
        ]
...
def send_passenger_complain_email(complain_details: Dict):
    ...
    try:
        ...
        if war_room_users:
            ...
            logging.info(
                f"No war room users found for depot {train_depo} in complaint {complain_details['complain_id']}"
            )
                    ...
                    if train_no in train_details:
                        for access in train_details[train_no]:
                            try:
                                ...
                                logging.warning(
                                    f"Date parsing error for user {user.get('id')}: {date_error}"
                                )
                                continue

                except (json.JSONDecodeError, TypeError) as json_error:
                    logging.warning(
                        f"JSON parsing error for user {user.get('id')}: {json_error}"
                    )
                    continue
    ...
    try:
        ...
        template_path = os.path.join(
            "templates", "complaint_creation_email_template.txt"
        )
        ...
        assigned_user_emails = [
            user.get("email") for user in assigned_users_list if user.get("email")
        ]
        assigned_user_emails = list(
            dict.fromkeys(assigned_user_emails)
        )  # Remove duplicates

        if assigned_user_emails:
            logging.info(
                f"Train access users to be notified: {', '.join(assigned_user_emails)}"
            )
            ...
            if email and not email.startswith("noemail") and "@" in email:
                try:
                    ...
                    if success:
                        emails_sent += 1
                        logging.info(
                            f"Email sent to {email} for complaint {complain_details['complain_id']}"
                        )
                    else:
        ...
        if not all_users_to_mail:
            logging.info(
                f"No users found for depot {train_depo} and train {train_no} in complaint {complain_details['complain_id']}"
            )
            return {
                "status": "success",
                "message": "No users found for this depot and train",
            }
        if not all_users_to_mail:
            logging.info(
                f"No users found for depot {train_depo} and train {train_no} in complaint {complain_details['complain_id']}"
