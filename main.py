...
from services import (
    create_complaint,
    get_complaint_by_id,
    get_complaints_by_date,
    update_complaint,
    delete_complaint,
    delete_complaint_media,
    upload_file_thread,
)
...
@app.get(
    "/rs_microservice/complaint/get/{complain_id}",
    response_model=RailSathiComplainResponse,
)
async def get_complaint(complain_id: int):
    ...
    try:
        ...
        return RailSathiComplainResponse(
            message="Complaint retrieved successfully", data=complaint
        )
...
@app.get(
    "/rs_microservice/complaint/get/date/{date_str}",
    response_model=List[RailSathiComplainResponse],
)
async def get_complaints_by_date_endpoint(
    date_str: str, mobile_number: Optional[str] = None
):
    ...
    try:
        ...
        if not mobile_number:
            raise HTTPException(
                status_code=400, detail="mobile_number parameter is required"
            )
...
async def create_complaint_endpoint_threaded(
    ...
    try:
        ...
        if rail_sathi_complain_media_files and len(rail_sathi_complain_media_files) > 0:
            ...
            for file_obj in rail_sathi_complain_media_files:
                if file_obj.filename:  # Check if file is actually uploaded
                    ...
                    logger.info(
                        f"Read file: {file_obj.filename}, size: {len(file_content)}"
                    )
        ...
        logger.info(
            f"Final complaint data retrieved with {len(updated_complaint.get('rail_sathi_complain_media_files', []))} media files"
        )
...
@app.patch(
    "/rs_microservice/complaint/update/{complain_id}",
    response_model=RailSathiComplainResponse,
)
async def update_complaint_endpoint(
    ...
    try:
        ...
        if pnr_number is not None:
            update_data["pnr_number"] = pnr_number
        if is_pnr_validated is not None:
            update_data["is_pnr_validated"] = is_pnr_validated
        if name is not None:
            update_data["name"] = name
        if mobile_number is not None:
            update_data["mobile_number"] = mobile_number
        if complain_type is not None:
            update_data["complain_type"] = complain_type
        if complain_description is not None:
            update_data["complain_description"] = complain_description
        if complain_date is not None:
            update_data["complain_date"] = complain_date
        if complain_status is not None:
            update_data["complain_status"] = complain_status
        if train_id is not None:
            update_data["train_id"] = train_id
        if train_number is not None:
            update_data["train_number"] = train_number
        if train_name is not None:
            update_data["train_name"] = train_name
        if coach is not None:
            update_data["coach"] = coach
        if berth_no is not None:
            update_data["berth_no"] = berth_no
        ...
        logger.info(
            f"Final complaint data retrieved with {len(final_complaint.get('rail_sathi_complain_media_files', []))} media files"
        )

        return {"message": "Complaint updated successfully", "data": final_complaint}
...
async def replace_complaint_endpoint(
    ...
    try:
        ...
        # Check permissions
        if (
            existing_complaint["created_by"] != name
            or existing_complaint["complain_status"] == "completed"
            or existing_complaint["mobile_number"] != mobile_number
        ):
            raise HTTPException(
                status_code=403,
                detail="Only user who created the complaint can update it.",
            )
        ...
        return {"message": "Complaint replaced successfully", "data": final_complaint}
...
async def delete_complaint_endpoint(
    complain_id: int, name: str = Form(...), mobile_number: str = Form(...)
):
    """Delete complaint"""
    try:
        ...
        if (
            ...
            raise HTTPException(
                status_code=403,
                detail="Only user who created the complaint can delete it.",
            )
...
async def delete_complaint_media_endpoint(
    ...
    try:
        logger.info(
            f"Deleting media files for complaint {complain_id} for user: {name}"
        )
        ...
        if not deleted_media_ids:
            raise HTTPException(
                status_code=400, detail="No media IDs provided for deletion."
            )
        ...
        if deleted_count == 0:
            raise HTTPException(
                status_code=400, detail="No matching media files found for deletion."
            )

        logger.info(
            f"{deleted_count} media file(s) deleted successfully for complaint {complain_id}"
        )
...
def get_train_details(train_no: str):
    ...
    try:
        cursor.execute(
            "SELECT * FROM trains_traindetails WHERE train_no = %s", (train_no,)
        )
        ...
        cursor.execute(
            "SELECT * FROM station_Depot WHERE depot_code = %s", (depot_code,)
        )
        ...
        if depot:
            ...
            cursor.execute(
                "SELECT * FROM station_division WHERE division_id = %s",
                (division_code,),
            )
            ...
            if division:
                ...
                cursor.execute(
                    "SELECT * FROM station_zone WHERE zone_id = %s", (zone_id,)
                )
